...
interface ItemDao{
    ...
    @Query("UPDATE item_table SET name =:name,weight=:weight, weightType=:weightType, price=:price, total =:total, drawableId =:drawableId, photoPath =:photoPath, listId = CAST(:listId AS BLOB) WHERE id = CAST(:id AS BLOB)")
    fun updateItem(id: UUID,name: String, weight: Float,weightType: String, price: Float, total: Float,listId: UUID,drawableId: Int, photoPath: String?)
