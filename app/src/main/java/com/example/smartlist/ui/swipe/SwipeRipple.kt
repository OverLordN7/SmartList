...
internal const val duration = 4_00
...
private fun linearInterpolation(start: Float, stop: Float, fraction: Float) = (start * (1- fraction) + stop * fraction)
...
internal class SwipeRippleState{
    ...
    suspend fun animate(
        ...
        val animationDurationMs = (duration * (if (action.isUndo) 1.75f else 1f)).roundToInt()
    ...
    fun draw(scope: DrawScope){
        ripple.value?.run {
            scope.clipRect {
                ...
                val radius = linearInterpolation(startRadius,endRadius, fraction = progress)
